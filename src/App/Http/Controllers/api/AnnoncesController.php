...
class AnnoncesController extends AmerController
{
    ...
    public static function getjob_by_Annonce_slug($annslug){
        ...
        if(!$annonce)return response()->json(['error'=>'']);
        ...
        ->get();
        if(!$JobData)return response()->json(['error'=>'']);
        $eldata=[];
    ...
    public function employment_apply_checknid($an_slug,$jbslug,$nid,Request $request){
        trim($nid);
        if(strlen($nid) !== 14){
            return ['result'=>'not14','message'=>trans("JOBLANG::apply.nid_phisical_error")];
        }
        $an= Employment_StartAnnonces::with('Employment_Stages')->where('Slug',$an_slug)->first();
        if(!$an){
            return ['result'=>'errorannonce','message'=>trans("JOBLANG::apply.nid_error_annonce")];
        }
        $job=Employment_Jobs::where('Slug',$jbslug)->where('Annonce_id',$an->id)->first();
        if(!$job){
            return ['result'=>'errorjob','message'=>trans("JOBLANG::apply.nid_error_annonce")];
        }
        $job_id=$job->id;
        ...
        return ['result'=>'error','message'=>trans("JOBLANG::apply.nid_error_annonce")];
    }
    function allgovs(){
